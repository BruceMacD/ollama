...
if which nvcc > /dev/null; then
  ...
  cmake -S ggml -B ggml/build/cuda-$CUDA_MAJOR_VERSION -DLLAMA_CUBLAS=on -DLLAMA_ACCELERATE=on -DLLAMA_K_QUANTS=on -DLLAMA_STATIC=on
  cmake --build ggml/build/cuda-$CUDA_MAJOR_VERSION --target server --config Release

  cmake -S gguf -B gguf/build/cuda-$CUDA_MAJOR_VERSION -DLLAMA_CUBLAS=on -DLLAMA_ACCELERATE=on -DLLAMA_K_QUANTS=on -DLLAMA_STATIC=on
